...
"""Stage 8: create train/validation splits for Stage 7 VQA conversations.
...
The script does not rewrite question/answer content. It only validates the new
Stage 7 answer schema, reports split statistics, and writes train.json,
validation.json, and dataset_stats.json.
"""
...
def selected_template_id(answer: Optional[Dict[str, Any]]) -> str:
    if not answer:
        return "missing_answer"
    mitigation = answer.get("scene_consistent_ego_mitigation", {})
    context = answer.get("scene_risk_context", {})
    return str(
        mitigation.get("selected_template_id")
        or context.get("selected_scene_template_id")
        or "missing_template"
    )
...
def clip_mitigation_consistency(entries: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    templates_by_clip: Dict[str, set] = defaultdict(set)
    ...
    template_violations = {
        ...
        for clip_id, values in templates_by_clip.items()
        if len(values) > 1
    }
    status_violations = {
        clip_id: sorted(values)
    ...
    return {
        "clips_checked": len(templates_by_clip),
        "template_consistency_violation_count": len(template_violations),
        "status_consistency_violation_count": len(status_violations),
        "payload_consistency_violation_count": len(payload_violations),
        "template_consistency_violation_examples": dict(list(template_violations.items())[:10]),
...
def make_stats(
    ...
    return {
        ...
        "samples_by_scene_template_train": count_answer_values(train_entries, train_answers, selected_template_id),
        "samples_by_scene_template_validation": count_answer_values(validation_entries, validation_answers, selected_template_id),
