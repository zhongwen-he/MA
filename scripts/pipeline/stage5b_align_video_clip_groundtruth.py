...
from agent_aliases import attach_identity
        ...
        for agent_id, agent_data in frame_agents.items():
            ...
            if offset < frame_count:
                agents[agent_id][frame_key] = attach_identity(
                    distance_only_groundtruth(agent_data), agent_id, agent_alias_map
                )
            else:
                agents[agent_id][frame_key] = risk_agent_to_groundtruth(agent_id, agent_data, agent_alias_map)
    return agents
...
def build_clip_entry(
    ...
    agent_alias_map = clip.get("agent_alias_map")
