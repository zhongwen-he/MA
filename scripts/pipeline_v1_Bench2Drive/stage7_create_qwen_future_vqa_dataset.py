...
frame". Bench2Drive actor ids and raw risk ids are written only to metadata sidecars
...
def build_question(agent_name: str, horizon_seconds: float) -> str:
    return (
        "<video>\n"
        "Analyze the 5-frame observed multi-camera Bench2Drive video clip. "
...
def create_metadata_entry(
    ...
    return {
        ...
        "target_agent": entry.get("target_agent", entry["target_vehicle"]),
...
def main() -> None:
    ...
    output = {
        "metadata": {
            "description": "Clean Qwen/LLaVA-style VQA dataset for 5-frame Bench2Drive video future-risk analysis",
            "format": "clean conversation pairs with multi-camera video paths",
            "task": "current risk, future risk, target-agent role, and scene-consistent quantitative ego meta-action mitigation",
    ...
    print(f"Skipped unnamed target agents: {skipped_unnamed}")
    print(f"Skipped ambiguous target agents: {skipped_ambiguous}")
