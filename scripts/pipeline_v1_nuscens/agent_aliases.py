...
"""Agent reference helpers for raw-video VQA generation.

Pipeline v1 keeps nuScenes instance tokens as internal join keys only. The VQA
...
All nuScenes categories can receive names. The final clip references are built
from Stage 4 close-agent risk rows, so far or irrelevant scene instances do not
become VQA targets merely because they exist in nuScenes.
"""
...
def is_vehicle_category(category_name: str) -> bool:
    return normalize_category_name(category_name).startswith("vehicle.")
