...
def enhanced_metric_to_agent(
    ...
    agent_alias_map: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    return risk_agent_to_groundtruth(agent_id, row_to_json_entry(enhanced_row(metric_row)), agent_alias_map)
...
def select_future_worst(
    ...
    for frame_index in future_frame_indices:
        ...
        if metric_row is None:
            continue
        agent = enhanced_metric_to_agent(agent_id, metric_row, agent_alias_map)
...
def risk_set_row(agent_id: str, agent_future: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "agent_id": identity.get("display_agent_id", agent_id),
        "display_agent_id": identity.get("display_agent_id", agent_id),
        "scene_agent_id": identity.get("scene_agent_id"),
...
def build_mitigation_explanation(
    ...
    shown_agent_id = display_id or agent_id
    if role == "triggering_risk_agent":
        result = (
            f"Target agent {shown_agent_id} is in the active risk set and directly contributes to the "
            f"scene-level ego meta-action {template_id}."
        )
    elif role == "monitoring_agent":
        result = (
            f"Target agent {shown_agent_id} is in the monitor set. It does not trigger active mitigation, "
            f"but inherits the same scene-level ego meta-action {template_id}."
        )
    elif role == "non_triggering_close_agent":
        result = (
            f"Target agent {shown_agent_id} remains above the monitor/intervention risk gates and inherits "
            f"the scene-level ego meta-action selected from other agents or maintain behavior."
        )
    else:
        result = (
            f"Target agent {shown_agent_id} has no available future risk label within the horizon and inherits "
...
def build_target_agent_future(
    ...
    identity = {
        "agent_id": current_agent.get("display_agent_id", current_agent.get("agent_id", agent_id)),
        "display_agent_id": current_agent.get("display_agent_id", current_agent.get("agent_id", agent_id)),
        "scene_agent_id": current_agent.get("scene_agent_id"),
        ...
        "alias_scope": current_agent.get("alias_scope"),
    }
    ...
    return {
        ...
        "agent_id": identity["display_agent_id"],
        "display_agent_id": identity["display_agent_id"],
...
def clip_future_summary(
    ...
    if rows:
        ...
        most_critical_display_agent = agent_identity.get("display_agent_id", agent_id)
        ...
        most_critical_display_agent = None
    ...
    return {
        ...
        "most_critical_display_agent": most_critical_display_agent,
...
def build_future_entry(
    ...
    agent_alias_map = entry.get("agent_alias_map")
    ...
    for agent_id, agent_future in target_agents.items():
        ...
        agent_future["mitigation_explanation"] = build_mitigation_explanation(
            ...
            agent_future.get("display_agent_id"),
        )
