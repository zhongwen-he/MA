...
copied to every target-vehicle label from the same clip. This is VQA
...
def select_clip_level_meta_action_legacy(
    ...
    if not triggered:
        ...
        basis = (
            "No target vehicle crosses the active intervention risk gate; monitor vehicles remain below active mitigation."
            if monitor_rows
            else "No target vehicle crosses the monitor or active intervention risk gates."
        )
    else:
        ...
        basis = (
            f"{len(triggered)} target vehicle(s) crossed the active risk gate. "
...
def select_clip_level_meta_action(
    ...
    if not risk_rows:
        mitigation_status = "monitor_or_prepare" if monitor_rows else "no_active_mitigation_required"
        basis = (
            "No target vehicle crosses the active risk gate within the 0.5s horizon; R6 keeps maintain behavior."
            if monitor_rows
            else "No target vehicle crosses the monitor or active intervention risk gates within the 0.5s horizon."
        )
    elif selected["template_id"] == R6_MAINTAIN_TEMPLATE_ID:
        mitigation_status = "active_mitigation_required"
        basis = (
            "At least one target vehicle crosses the active risk gate, but every non-maintain candidate failed "
...
def build_mitigation_explanation(
    ...
    shown_agent = reference_name or "the target vehicle"
    if role == "triggering_risk_agent":
        result = (
            f"The target vehicle {shown_agent} is in the active risk set and directly contributes to the "
            f"scene-level ego meta-action {template_id}."
        )
    elif role == "monitoring_agent":
        result = (
            f"The target vehicle {shown_agent} is in the monitor set. It does not trigger active mitigation, "
            f"but inherits the same scene-level ego meta-action {template_id}."
        )
    elif role == "non_triggering_close_agent":
        result = (
            f"The target vehicle {shown_agent} remains above the monitor/intervention risk gates and inherits "
            f"the scene-level ego meta-action selected from other agents or maintain behavior."
        )
    else:
        result = (
            f"The target vehicle {shown_agent} has no available future risk label within the horizon and inherits "
...
def future_selection_explanation(
    ...
    if future_worst is None:
        result = "No future relative metric is available for this target vehicle within the selected horizon."
    else:
        result = (
    ...
    return {
        "step_1": f"Evaluate the same target vehicle over the next {horizon_seconds:.1f} seconds.",
