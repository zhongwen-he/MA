...
Stage 7 now emits one VQA sample per named target vehicle, while all target-vehicle
...
CURRENT_TASK_NAME = "current risk, future risk, target-vehicle role, and scene-consistent quantitative ego meta-action mitigation"
...
def mitigation_status(answer: Optional[Dict[str, Any]]) -> str:
    ...
    action = answer.get("ego_meta_action", {})
    return f"{action.get('longitudinal', 'missing_longitudinal')}__{action.get('lateral', 'missing_lateral')}"
...
def future_risk_score(answer: Optional[Dict[str, Any]]) -> str:
    if not answer:
        return "missing_answer"
    future = answer.get("future_worst_risk", {})
...
def validate_new_answer_schema(entries: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for index, entry in enumerate(entries):
        ...
        if not answer.get("ego_meta_action"):
            missing_ego_meta_action.append(entry_id)
        ...
        expected_count = answer.get("final_timestep")
        if not isinstance(distances, list) or len(distances) != expected_count:
            incomplete_distance_sequences.append(
                {"id": entry_id, "final_timestep": expected_count, "distance_count": len(distances) if isinstance(distances, list) else None}
            )

        if "quantitative_mitigation_suggestion" in answer or "scene_consistent_ego_mitigation" in answer:
    ...
    return {
        ...
        "missing_ego_meta_action_count": len(missing_ego_meta_action),
        ...
        "missing_ego_meta_action_examples": missing_ego_meta_action[:10],
...
def clip_meta_action_consistency(entries: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    statuses_by_clip: Dict[str, set] = defaultdict(set)
    action_payloads_by_clip: Dict[str, set] = defaultdict(set)
    ...
    for entry in entries:
        ...
        payload = {
            "ego_meta_action": (answer or {}).get("ego_meta_action", {}),
            "quantitative_suggestion": (answer or {}).get("quantitative_suggestion", {}),
        }
        statuses_by_clip[clip_id].add(status)
        action_payloads_by_clip[clip_id].add(json.dumps(payload, sort_keys=True, ensure_ascii=False))
    ...
    payload_violations = {
        clip_id: len(values)
        for clip_id, values in action_payloads_by_clip.items()
        if len(values) > 1
    }
    return {
...
def make_stats(
    ...
    return {
        ...
        "split_policy_note": "clip split keeps all target-vehicle VQA samples from the same clip in one split so they share the same scene-consistent mitigation label without train/validation leakage.",
        ...
        "clip_meta_action_consistency": clip_meta_action_consistency(all_entries),
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="clip",
        help="Default clip keeps all target-vehicle samples from the same clip in one split. Use entry for legacy NuRisk-style sample-level shuffling.",
    )
...
def main() -> None:
    ...
    consistency = stats["clip_meta_action_consistency"]
    if consistency["payload_consistency_violation_count"]:
        print("Warning: some clips have inconsistent ego meta-action payloads; see dataset_stats.json.")
