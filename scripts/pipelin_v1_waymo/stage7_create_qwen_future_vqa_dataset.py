...
as "the closest vehicle ahead-left in the adjacent lane at the last observed
frame". Waymo ids and raw risk ids are written only to metadata sidecars for
...
def build_question(agent_name: str, horizon_seconds: float) -> str:
    return (
        "<video>\n"
        "Analyze the 5-frame observed multi-camera Waymo video clip. "
...
def main() -> None:
    ...
    output = {
        "metadata": {
            "description": "Clean Qwen/LLaVA-style VQA dataset for 5-frame Waymo video future-risk analysis",
            "format": "clean conversation pairs with multi-camera video paths",
            "task": "current risk, future risk, target-vehicle role, and scene-consistent quantitative ego meta-action mitigation",
